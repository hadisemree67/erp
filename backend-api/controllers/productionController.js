...
// Production controller auto-generated
...
class ProductionController {
  ...
  async postMachines(req, res) {
    ...
    try {
      ...
      const [result] = await db.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
        `, [name, last_maintenance || null, 'Boş', machine_code || null, max_capacity || null, min_capacity || null, catsJson, prep_time_minutes || 0, alternative_machine_id || null, supplier_name || null, supplier_email || null, supplier_phone || null, period, next_maintenance]);
  ...
  async putMachinesIdStatus(req, res) {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Makine ID.'
    });
    const {
      status
    } = req.body;
  ...
  async putMachinesId(req, res) {
    ...
    try {
      ...
      const [result] = await db.query(`
            ...
            WHERE id = ?
        `, [name, last_maintenance || null, machine_code || null, max_capacity || null, min_capacity || null, catsJson, prep_time_minutes || 0, alternative_machine_id || null, supplier_name || null, supplier_email || null, supplier_phone || null, period, next_maintenance, id]);
      if (result.affectedRows === 0) return res.status(404).json({
        success: false,
        message: 'Makine bulunamadı.'
      });
  ...
  async postMachinesIdReportIssue(req, res) {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Makine ID.'
    ...
    try {
      ...
      res.json({
        success: true,
        message: emailSent ? 'Arıza bildirim e-postası bakımcıya/satıcıya iletildi ve makine durumu "Arızalı" olarak güncellendi.' : 'Makine durumu "Arızalı" olarak güncellendi. (Makine için kayıtlı e-posta adresi bulunamadığından veya mail sunucusu ayarlanmadığından e-posta gönderilemedi).'
      });
    } catch (err) {
  ...
  async deleteMachinesId(req, res) {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Makine ID.'
  ...
  async postOrdersMatch(req, res) {
    const {
      product_id,
      planned_quantity
    } = req.body;
    if (!product_id || !planned_quantity) return res.status(400).json({
      success: false,
      message: 'Ürün ve miktar gerekli.'
    });
    try {
      ...
      if (prodRows.length === 0) return res.status(404).json({
        success: false,
      ...
      try {
        ...
        console.warn('JSON Parse Error (Formula/production):', e.message);
      }
      ...
      const matched = machines.filter(m => {
        ...
        if (m.allowed_categories) {
          try {
            ...
            console.warn('JSON Parse Error (allowed_categories):', e.message);
          }
        }
        return true;
      });
      if (matched.length === 0) {
        return res.status(400).json({
          success: false,
          message: 'Bu ürünü ve miktarı işleyebilecek uygun makine/kazan bulunamadı. Lütfen kapasiteleri ve kategorileri kontrol edin.'
        });
      }
  ...
  async postOrders(req, res) {
    ...
    try {
      ...
      try {
        ...
        console.warn('JSON Parse Error (Formula check):', e.message);
      }
      ...
      for (const step of formula) {
        for (const mat of step.materials || []) {
          if (!aggregatedMaterials[mat.material]) {
            aggregatedMaterials[mat.material] = {
              quantity: 0,
              unit: mat.unit || ''
            };
          }
      ...
      res.json({
        success: true,
        message: 'Üretim emri oluşturuldu, toplama aşamasına geçilebilir.',
        orderId
      });
    } catch (err) {
      await connection.rollback();
  ...
  async deleteOrdersId(req, res) {
    try {
      const id = parseInt(req.params.id, 10);
      if (isNaN(id)) return res.status(400).json({
        success: false,
        message: 'Geçersiz Emir ID.'
      });
      ...
      if (rows[0].status === 'Üretimde' || rows[0].status === 'Tamamlandı') {
        return res.status(400).json({
          success: false,
          message: 'Üretimde veya tamamlanmış sipariş iptal edilemez.'
        });
      }
      ...
      res.json({
        success: true,
        message: 'Sipariş başarıyla iptal edildi ve silindi.'
      });
    } catch (err) {
  ...
  async postOrdersIdArchive(req, res) {
    const connection = await db.getConnection();
    try {
      await connection.beginTransaction();
      ...
      res.json({
        success: true,
        message: 'Sipariş arşivlendi ve loglandı.'
      });
    } catch (err) {
      await connection.rollback();
  ...
  async postOrdersIdPick(req, res) {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Emir ID.'
  ...
  async postOrdersIdStart(req, res) {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Emir ID.'
    ...
    try {
      ...
      res.json({
        success: true,
        message: 'Üretim başladı, birinci adım işletilebilir.'
      });
    } catch (err) {
      await connection.rollback();
      res.status(500).json({
  ...
  async postOrdersIdStepsStepIdStart(req, res) {
    const id = parseInt(req.params.id, 10);
    const step_id = parseInt(req.params.step_id, 10);
    if (isNaN(id) || isNaN(step_id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Emir veya Adım ID.'
    });
    const connection = await db.getConnection();
    try {
      await connection.beginTransaction();
  ...
  async postOrdersIdStepsStepIdVerifyMaterial(req, res) {
    const id = parseInt(req.params.id, 10);
    const step_id = parseInt(req.params.step_id, 10);
    if (isNaN(id) || isNaN(step_id)) return res.status(400).json({
      success: false,
    ...
    try {
      ...
      try {
        ...
        console.warn('JSON Parse Error (verified_materials):', e.message);
      }
  ...
  async postOrdersIdStepsStepIdComplete(req, res) {
    const id = parseInt(req.params.id, 10);
    const step_id = parseInt(req.params.step_id, 10);
    if (isNaN(id) || isNaN(step_id)) return res.status(400).json({
      success: false,
  ...
  async postOrdersIdComplete(req, res) {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.status(400).json({
      success: false,
      message: 'Geçersiz Emir ID.'
    ...
    try {
      ...
      if (!is_manager_approval) {
        if (waste_pct > 5 && waste_pct <= 30 && !waste_reason) {
          return res.status(400).json({
            success: false,
            requireReason: true,
            message: 'Fire oranı %5-%30 arasında. Lütfen bir neden giriniz.'
          });
        }
        if (waste_pct > 30) {
          ...
          return res.json({
            success: true,
            requireManager: true,
            message: 'Fire oranı %30 üzeri! Yönetici onayı bekleniyor.'
          });
        }
      } else {
        ...
        if (!manager_explanation) {
          return res.status(400).json({
            success: false,
            message: 'Yönetici açıklaması zorunludur.'
          });
        }
      }
      ...
      res.json({
        success: true,
        message: 'Üretim tamamlandı, depo onayına gönderildi.'
      });
    } catch (err) {
      await connection.rollback();
  ...
  async getWarehouseUsers(req, res) {
    try {
  ...
  async getWarehouseAcceptances(req, res) {
    try {
      const [rows] = await db.query(`
            SELECT 
                po.*, 
  ...
  async postOrdersIdAcceptDelivery(req, res) {
    try {
      const [result] = await db.query(`
            UPDATE production_orders 
  ...
  async postOrdersIdReportDefect(req, res) {
    const {
      received_quantity,
      reason
    } = req.body;
    ...
    try {
      ...
      res.json({
        success: true,
        message: 'Hatalı teslimat raporlandı ve sağlam ürünler teslim alındı.'
      });
    } catch (err) {
      await connection.rollback();
  ...
  async postOrdersIdWarehouseStockEntry(req, res) {
    const {
      shelfAllocations,
      batch_number,
      expiration_date
    ...
    try {
      ...
      for (const allocation of shelfAllocations) {
        ...
        if (productVolume > 0 && shelfCode) {
          ...
          if (maxVolume > 0) {
            const [filledData] = await connection.query('SELECT COALESCE(SUM(b.quantity * p.Volume), 0) as filled FROM wms_stock_balances b JOIN products p ON b.product_id = p.Id WHERE b.warehouse_id = ? AND b.shelf_code = ? FOR UPDATE', [warehouseId, shelfCode]);
  ...
  async getMaxQuantityProductId(req, res) {
    try {
      ...
      try {
        ...
        console.warn('JSON Parse Error (Formula packing):', e.message);
      }
      if (!formula || formula.length === 0) return res.json({
        success: false,
        message: 'Reçete yok.'
      });
      const aggregatedMaterials = {};
      ...
      for (const [materialName, matData] of Object.entries(aggregatedMaterials)) {
        ...
        if (fromUnit !== productUnit && fromUnit !== '') {
          ...
          if (isFromSmall && isToLarge) reqQtyForOne = reqQtyRaw / 1000;else if (isFromLarge && isToSmall) reqQtyForOne = reqQtyRaw * 1000;else if (fromUnit === 'tank' && isToLarge) reqQtyForOne = reqQtyRaw * 10;
        }
          ...
          for (const m of machines) {
            if (m.allowed_categories) {
              try {
                ...
                console.warn('JSON Parse Error (packaging_info):', e.message);
              }
            }
          }
        }
  ...
  async getCapacityAnalysisProductId(req, res) {
    const {
      productId
    } = req.params;
    try {
      ...
      if (!Array.isArray(formula) || formula.length === 0) {
        return res.json({
          ...
          data: {
            hasFormula: false,
            message: 'Bu ürün için reçete (BOM) tanımlanmamış.'
          }
        });
      }
      ...
      res.status(500).json({
        success: false,
        message: 'Kapasite analizi yapılamadı.'
      });
    }
  }
  async getRequests(req, res) {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Üretim talepleri alınamadı.'
      });
    }
  }
  async postRequests(req, res) {
    ...
    try {
      ...
      for (let i = 0; i < quantitiesToRequest.length; i++) {
        ...
        await db.query('INSERT INTO production_requests (product_id, requested_quantity, source, creator, reason, status, priority) VALUES (?, ?, ?, ?, ?, ?, ?)', [productId, qtyToRequest, 'Manuel', creator, finalReason, 'Bekleyen', req.body.priority || 'Normal']);
      }
  ...
  async putRequestsIdStatus(req, res) {
    const {
      id
    } = req.params;
    const {
    ...
    try {
      if (status === 'Reddedildi') {
        ...
        res.json({
          success: true,
          message: 'Talep reddedildi ve listeden silindi.'
        });
      } else {
        ...
        res.json({
          success: true,
          message: `Talep durumu ${status} olarak güncellendi.`
        });
      }
    } catch (error) {
  ...
  async putProductStockRulesProductId(req, res) {
    const {
      productId
    } = req.params;
    const {
    ...
    try {
      const [result] = await db.query('UPDATE products SET critical_stock_level = ?, minimum_production_quantity = ? WHERE Id = ?', [critical_stock_level, minimum_production_quantity, productId]);
      if (result.affectedRows === 0) return res.status(404).json({
        success: false,
        message: 'Ürün bulunamadı.'
      });
      ...
      res.json({
        success: true,
        message: 'Kritik stok kuralları güncellendi.'
      });
    } catch (error) {
  ...
  async postTriggerStockCheck(req, res) {
