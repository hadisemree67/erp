...
class WmsController {
  async getWarehouses(req, res) {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Depolar getirilirken hata oluştu.'
      });
    }
  }
  async getWarehousesWarehouseIdLocations(req, res) {
    try {
      const warehouseId = parseInt(req.params.warehouseId, 10);
      if (isNaN(warehouseId)) return res.status(400).json({
        success: false,
        message: 'Geçersiz Depo ID.'
      });
      const [rows] = await db.query('SELECT id, shelf_code as shelf, shelf_code as rack, "Genel" as aisle, shelf_code as barcode FROM warehouse_shelves WHERE warehouse_id = ?', [warehouseId]);
      res.json(rows);
    } catch (error) {
  ...
  async postWarehousesWarehouseIdLayout(req, res) {
    try {
      const warehouseId = parseInt(req.params.warehouseId, 10);
      if (isNaN(warehouseId)) return res.status(400).json({
        success: false,
      ...
      res.status(500).json({
        success: false,
        message: 'Kroki kaydedilirken hata oluştu.'
      });
    }
  }
  async getWarehousesWarehouseIdProductsProductIdShelves(req, res) {
    try {
      const warehouseId = parseInt(req.params.warehouseId, 10);
      const productId = parseInt(req.params.productId, 10);
      if (isNaN(warehouseId) || isNaN(productId)) return res.status(400).json({
        success: false,
        message: 'Geçersiz Depo veya Ürün ID.'
      });
      const [rows] = await db.query('SELECT DISTINCT shelf_code FROM wms_stock_balances WHERE warehouse_id = ? AND product_id = ? AND quantity > 0', [warehouseId, productId]);
      res.json({
        success: true,
        data: rows.map(r => r.shelf_code)
      });
    } catch (err) {
  ...
  async getWarehousesWarehouseIdLayout(req, res) {
    try {
      const warehouseId = parseInt(req.params.warehouseId, 10);
      if (isNaN(warehouseId)) return res.status(400).json({
        success: false,
      ...
      res.status(500).json({
        success: false,
        message: 'Kroki getirilirken hata oluştu.'
      });
    }
  }
  async getWarehousesWarehouseIdShelvesShelfCodeStock(req, res) {
    try {
      const warehouseId = parseInt(req.params.warehouseId, 10);
      if (isNaN(warehouseId)) return res.status(400).json({
        success: false,
      ...
      const shelfCode = req.params.shelfCode;
      ...
      res.status(500).json({
        success: false,
        message: 'Raf detayları getirilirken hata oluştu.'
      });
    }
  }
  async postWarehousesWarehouseIdShelvesShelfCodeClear(req, res) {
    ...
    try {
      ...
      for (let row of stockRows) {
        // Movement
        await clearConn.query('INSERT INTO StockMovements (ProductId, UserId, MovementType, Quantity, warehouse_id, shelf_code, batch_number, expiration_date, Description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [row.product_id, userId, 'OUT', row.quantity, warehouseId, shelfCode, row.batch_number, row.expiration_date, 'Raf Sıfırlama (Boşaltma)']);
      ...
      res.json({
        success: true,
        message: 'Raf başarıyla boşaltıldı.'
      });
    } catch (error) {
      ...
      res.status(500).json({
        success: false,
        message: 'Raf boşaltılırken hata oluştu.'
      });
    }
  }
  async postStockEntry(req, res) {
    const {
      productId,
      warehouseId,
      shelfAllocations,
    ...
    if (!productId || !warehouseId || !shelfAllocations || !Array.isArray(shelfAllocations) || shelfAllocations.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'Ürün, depo, ve en az bir raf tahsisi zorunludur.'
      });
    }
    ...
    try {
      ...
      if (warehouseCheck.length === 0) {
        ...
        return res.status(404).json({
          success: false,
          message: 'Belirtilen depo bulunamadı.'
        });
      }
      ...
      for (const allocation of shelfAllocations) {
        if (!allocation.shelfCode || !allocation.quantity || allocation.quantity <= 0) {
          ...
          return res.status(400).json({
            success: false,
            message: 'Raf ve geçerli bir miktar girilmelidir.'
          });
        }
        ...
        if (shelfCheck.length === 0) {
          ...
          return res.status(404).json({
            success: false,
            message: `Hata: ${allocation.shelfCode} kodlu raf bulunamadı!`
          });
        }
        ...
        if (productVolume > 0 || packageCapacity > 0) {
          ...
          if (sW > 0 && sH > 0 && sD > 0 && pW > 0 && pH > 0 && pD > 0) {
            const [filledData] = await conn.query('SELECT quantity, p.package_capacity FROM wms_stock_balances b LEFT JOIN products p ON b.product_id = p.Id WHERE b.warehouse_id = ? AND b.shelf_code = ? FOR UPDATE', [warehouseId, allocation.shelfCode]);
            ...
            if (absoluteMaxCapacity > 0) {
              const [filledData] = await conn.query('SELECT COALESCE(SUM(quantity), 0) as filled FROM wms_stock_balances WHERE warehouse_id = ? AND shelf_code = ? AND product_id = ? FOR UPDATE', [warehouseId, allocation.shelfCode, productId]);
      ...
      for (const allocation of shelfAllocations) {
        ...
        // Açıklama alanını 255 karakter kısıtlamasına göre sınırlandır (Data too long hatasını engellemek için)
        const movementDesc = (description || 'Depo Girişi (Mal Kabul)').substring(0, 250);

        // 1. Log the movement
        await conn.query('INSERT INTO StockMovements (ProductId, UserId, MovementType, Quantity, warehouse_id, shelf_code, batch_number, expiration_date, Description, supplier_id, unit_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [productId, userId, 'IN', quantity, warehouseId, shelfCode, batchNumber || null, formattedExpDate, movementDesc, supplierId || null, unitPrice || null]);
        ...
        const [existingBalances] = await conn.query('SELECT id, quantity FROM wms_stock_balances WHERE product_id = ? AND warehouse_id = ? AND shelf_code = ? AND COALESCE(batch_number, "") = ? AND (expiration_date = ? OR (expiration_date IS NULL AND ? IS NULL)) AND (supplier_id = ? OR (supplier_id IS NULL AND ? IS NULL)) AND (unit_price = ? OR (unit_price IS NULL AND ? IS NULL)) FOR UPDATE', [productId, warehouseId, shelfCode, safeBatch, safeExp, safeExp, supplierId || null, supplierId || null, unitPrice || null, unitPrice || null]);
        if (existingBalances.length > 0) {
          await conn.query('UPDATE wms_stock_balances SET quantity = quantity + ? WHERE id = ?', [quantity, existingBalances[0].id]);
        } else {
          await conn.query('INSERT INTO wms_stock_balances (product_id, warehouse_id, shelf_code, batch_number, expiration_date, quantity, supplier_id, unit_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [productId, warehouseId, shelfCode, safeBatch, safeExp, quantity, supplierId || null, unitPrice || null]);
        }
      }
      ...
      if (supplierId && unitPrice && unitPrice > 0 && totalAdded > 0) {
        ...
        const desc = `${prodName} ürünü için ${supName} adlı tedarikçiden ${totalAdded} adet manuel depo girişi (mal kabul) yapıldı.`.substring(0, 250);
        await conn.query(`
      ...
      res.status(500).json({
        success: false,
        message: 'Stok işlemi sırasında hata oluştu.'
      });
    }
  }
  ...
  async putStockId(req, res) {
    ...
    try {
      ...
      // Find old balance
      const [rows] = await conn.query('SELECT * FROM wms_stock_balances WHERE id = ? FOR UPDATE', [id]);
      if (rows.length === 0) {
        await conn.query('ROLLBACK');
        conn.release();
        return res.status(404).json({
      ...
      if (warehouse_id_val !== oldData.warehouse_id || shelf_code_val !== oldData.shelf_code) {
        const [shelfCheck] = await conn.query('SELECT id FROM warehouse_shelves WHERE warehouse_id = ? AND shelf_code = ?', [warehouse_id_val, shelf_code_val]);
        if (shelfCheck.length === 0) {
          await conn.query('ROLLBACK');
          conn.release();
          return res.status(404).json({
  ...
  async postTransfer(req, res) {
    ...
    try {
      ...
      const [targetShelfCheck] = await conn.query('SELECT id FROM warehouse_shelves WHERE warehouse_id = ? AND shelf_code = ?', [targetWarehouseId, targetShelfCode]);
      ...
      if (targetRows.length > 0) {
        await conn.query('UPDATE wms_stock_balances SET quantity = quantity + ? WHERE id = ?', [transferQty, targetRows[0].id]);
      } else {
  ...
  async getShelfCapacity(req, res) {
    const {
      warehouseId,
      shelfCode,
      productId,
    ...
    try {
      ...
      const [filledRows] = await db.query('SELECT b.product_id, b.quantity, p.Volume, p.package_capacity FROM wms_stock_balances b JOIN products p ON b.product_id = p.Id WHERE b.warehouse_id = ? AND b.shelf_code = ?', [warehouseId, shelfCode]);
  ...
  async getWarehouseCapacities(req, res) {
    const {
      warehouseId,
      productId,
      w,
    ...
    if (!warehouseId) return res.status(400).json({
      success: false,
      message: 'Eksik parametre.'
    });
    try {
      ...
      const [filledData] = await db.query('SELECT b.shelf_code, b.product_id, b.quantity, p.Volume as prodVolume, p.package_capacity FROM wms_stock_balances b LEFT JOIN products p ON b.product_id = p.Id WHERE b.warehouse_id = ?', [warehouseId]);
      const filledVolMap = {};
      const filledPkgMap = {};
      ...
      filledData.forEach(r => {
        ...
        if (productId && r.product_id && r.product_id.toString() === productId.toString() && productVolume > 0) {
          vol = productVolume;
        }
        let pCapRow = parseFloat(r.package_capacity) || 1;
        if (pCapRow <= 0) pCapRow = 1;
        const pkgs = Math.ceil((parseFloat(r.quantity) || 0) / pCapRow);
        const sCode = (r.shelf_code || '').trim();
        filledVolMap[sCode] = (filledVolMap[sCode] || 0) + pkgs * vol;
        filledPkgMap[sCode] = (filledPkgMap[sCode] || 0) + pkgs;
        if (r.product_id && productId && r.product_id.toString() === productId.toString() && parseFloat(r.quantity) > 0) {
          containsSameProductMap[sCode] = true;
          const corridor = sCode.split('-')[0].trim();
          existingCorridors.add(corridor);
        }
        if (productId && r.product_id && r.product_id.toString() !== productId.toString() && parseFloat(r.quantity) > 0) {
          containsOtherProductsMap[sCode] = true;
        }
      });
      const capacities = {};
      shelvesData.forEach(shelf => {
        const sCode = (shelf.shelf_code || '').trim();
        ...
        const currentFilledVol = filledVolMap[sCode] || 0;
        const currentPackages = filledPkgMap[sCode] || 0;
        const calc = calculateShelf3D({
          sW,
          sH,
          sD,
        ...
        capacities[shelf.shelf_code] = {
          ...
          hasSameProduct: containsSameProductMap[sCode] || false,
          hasSameCorridor: existingCorridors.has(corridor),
          hasOtherProducts: containsOtherProductsMap[sCode] || false
        };
      });
      res.json({
        success: true,
  ...
  async getShelfByBarcode(req, res) {
    const {
      barcode
    } = req.query;
    if (!barcode) {
      return res.status(400).json({
        success: false,
        message: 'Barkod parametresi eksik.'
      });
    }
    try {
      const [rows] = await db.query('SELECT s.warehouse_id, s.shelf_code, w.name as warehouse_name FROM warehouse_shelves s JOIN warehouses w ON s.warehouse_id = w.id WHERE s.barcode = ? LIMIT 1', [barcode]);
      if (rows.length > 0) {
        res.json({
          success: true,
          data: rows[0]
        ...
        res.json({
          success: false,
          message: 'Bu barkoda ait raf bulunamadı.'
        });
      }
    } catch (error) {
  ...
  async postDeductFefo(req, res) {
    ...
    try {
      ...
      const [balances] = await conn.query(balancesQuery, queryParams);
      ...
      for (const balance of balances) {
        ...
        if (balance.quantity === qtyToTake) {
          await conn.query('DELETE FROM wms_stock_balances WHERE id = ?', [balance.id]);
        } else {
          await conn.query('UPDATE wms_stock_balances SET quantity = quantity - ? WHERE id = ?', [qtyToTake, balance.id]);
        }
...
module.exports = new WmsController();
