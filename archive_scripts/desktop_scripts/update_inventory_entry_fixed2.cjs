/*
 * ÖZET:
 * Bu script, tedarikçi listesi ve Kritik Stok Seviyesi gibi UI elemanlarının 
 * dizilimini (grid vs.) iyileştirmek için ikinci bir düzenleme (fix) sağlar.
 */
const fs = require('fs');
...
if (!code.includes('MULTIPLE SUPPLIERS SECTION')) {
    ...
    if (tedarikIdx !== -1) {
        // Grid başlangıcı: <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }}>`
        // Veya etiketin hemen öncesindeki <div style={{ display: 'flex', flexDirection: 'column' }}> kısmını bulabiliriz.
        const gridStartIdx = code.lastIndexOf('<div style={{ display: \'grid\'', tedarikIdx);
        
        if (gridStartIdx !== -1) {
            // Bu bloğun sonunu bul. "Kritik Stok Seviyesi" etiketinin hemen öncesinde biter.
            ...
            if (kritikIdx !== -1) {
                ...
                if (kritikDivIdx !== -1) {
                    ...
                    console.log('kritikDivIdx not found');
                }
            } else {
                console.log('kritikIdx not found');
            }
        } else {
            console.log('gridStartIdx not found');
        }
    }
} else {
