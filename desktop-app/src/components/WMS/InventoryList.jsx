...
const InventoryList = ({ currentUser, initialEntryVisible = false }) => {
  ...
  // Modals for editing product & adding stock
  const [isEditingProduct, setIsEditingProduct] = useState(false);
  const [editingProductData, setEditingProductData] = useState(null);
  
  const [isAddingStock, setIsAddingStock] = useState(false);
  const [addingStockMaterialName, setAddingStockMaterialName] = useState('');
  ...
  const fetchStockList = async () => {
    ...
    try {
      ...
      const materials = Array.isArray(prodData) ? prodData.filter(p => p.Category === 'Hammadde') : [];
      const stocks = stockData.success ? (stockData.data || []) : [];

      const grouped = materials.map(material => {
          const productBatches = stocks.filter(s => s.product_id === material.Id);
          const totalWmsQty = productBatches.reduce((acc, b) => acc + b.quantity, 0);
          return {
              groupKey: String(material.Id),
              product: material,
  ...
  const submitEditStock = async (e) => {
    ...
    const uniqueWarehouses = [...new Set(groupedItems.flatMap(g => g.batches.map(b => b.warehouse_name)))].filter(Boolean).sort();

    if (isEditingProduct) {
        return (
            <InventoryEntry 
                currentUser={currentUser} 
                editItem={editingProductData} 
                onCancel={() => setIsEditingProduct(false)}
                onSuccess={() => { setIsEditingProduct(false); fetchStockList(); }}
            />
        );
    }
    ...
    if (isAddingStock) {
        ...
        return (
            <div style={{ position: 'relative' }}>
                <button 
                    onClick={() => { setIsAddingStock(false); fetchStockList(); }} 
    ...
    const handleDeleteProduct = async (product) => {
        ...
        try {
            ...
            alert('Sunucu hatası veya ürünün bağlı olduğu stok hareketleri var.');
        }
    };
                ...
                return (
                    <tr key={group.groupKey} className="hover-row" onClick={() => setSelectedGroup(group)} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background-color 0.15s', cursor: 'pointer', backgroundColor: 'white' }} onMouseOver={e => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseOut={e => e.currentTarget.style.backgroundColor = 'white'}>
                        ...
                        <td style={{ padding: '16px', color: '#475569', fontSize: '14px' }}>
                            {(() => {
                                const suppliers = [...new Set((group.product?.suppliers || []).map(s => s.SupplierName).filter(Boolean))];
                        ...
                        <td style={{ padding: '16px', textAlign: 'right', display: 'flex', gap: '8px', justifyContent: 'flex-end', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', backgroundColor: '#f8fafc', padding: '4px 8px', borderRadius: '6px', border: '1px solid #e2e8f0', marginRight: '16px' }} onClick={e => e.stopPropagation()}>
                                  <button onClick={() => { setAddingStockMaterialName(group.product_name); setIsAddingStock(true); }} title="Yeni Stok Ekle" style={{ background: 'none', border: 'none', color: '#334155', cursor: 'pointer', padding: '4px', transition: 'color 0.2s' }} onMouseOver={e => e.currentTarget.style.color = '#10b981'} onMouseOut={e => e.currentTarget.style.color = '#334155'}>
                                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>
                                  </button>
                                  <button onClick={() => { setEditingProductData(group); setIsEditingProduct(true); }} title="Ürün Bilgilerini Düzenle" style={{ background: 'none', border: 'none', color: '#334155', cursor: 'pointer', padding: '4px', transition: 'color 0.2s' }} onMouseOver={e => e.currentTarget.style.color = '#3b82f6'} onMouseOut={e => e.currentTarget.style.color = '#334155'}>
                                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                  </button>
                                  <button onClick={() => handleDeleteProduct(group)} title="Ürünü Sil (Tüm stoklarla birlikte)" style={{ background: 'none', border: 'none', color: '#334155', cursor: 'pointer', padding: '4px', transition: 'color 0.2s' }} onMouseOver={e => e.currentTarget.style.color = '#ef4444'} onMouseOut={e => e.currentTarget.style.color = '#334155'}>
              ...
              >
                  ...
                  <div style={{ padding: '24px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      {selectedGroup.batches.length === 0 ? (
                          <div style={{ padding: '24px', textAlign: 'center', backgroundColor: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '8px' }}>
                              <div style={{ color: '#64748b', fontSize: '14px', marginBottom: '8px' }}>Bu ürün henüz WMS sistemine (depo raflarına) alınmamış.</div>
                              <div style={{ color: '#0f172a', fontWeight: '600', fontSize: '15px' }}>Gösterilen stok, ürün oluşturulurken girilen "İlk Stok" miktarıdır.</div>
                              <div style={{ marginTop: '16px', fontSize: '13px', color: '#64748b', lineHeight: '1.5' }}>
                                  Lütfen bu stoğu yönetebilmek (satış, üretim, sayım) için <strong>Stok Girişi</strong> butonuyla sisteme bir depo ve raf ataması yapınız.
                              </div>
                          </div>
                      ) : (
                          selectedGroup.batches.map((batch, index) => {
                              ...
                              return (
                                  <div key={batch.balance_id} style={{ padding: '20px', border: '1px solid #e2e8f0', borderRadius: '8px', backgroundColor: '#fff', position: 'relative' }}>
                                      ...
                                      <div>
                                          <div style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '4px' }}>Tedarikçi</div>
                                          <div style={{ color: '#0f172a', fontWeight: '500' }}>{batch.supplier_name || '-'}</div>
                                      </div>
                                      <div>
