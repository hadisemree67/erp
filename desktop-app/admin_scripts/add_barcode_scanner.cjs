...
function updateEntryFile(filename) {
    ...
    // Fix useEffect to preserve shelfCode if it is valid for the new warehouse
    ...
    // Add barcode scanner UI above Depo Seçiniz
                        ...
                        <input 
                            ...
                            onKeyDown={async (e) => {
                                if (e.key === 'Enter' && e.target.value.trim() !== '') {
                                    ...
                                    try {
                                        // apiFetch requires importing if not already, but apiFetch is used in both files
