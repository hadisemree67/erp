...
if (!code.includes('MULTIPLE SUPPLIERS SECTION')) {
    ...
    if (tedarikIdx !== -1) {
        // The start of the grid is `<div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }}>`
        // Or we can just find `<div style={{ display: 'flex', flexDirection: 'column' }}>` right before the label.
        const gridStartIdx = code.lastIndexOf('<div style={{ display: \'grid\'', tedarikIdx);
        
        if (gridStartIdx !== -1) {
            // Find the end of this block. It ends right before "Kritik Stok Seviyesi" label.
            const kritikIdx = code.indexOf('Kritik Stok Seviyesi', gridStartIdx);
            if (kritikIdx !== -1) {
                // Find the div containing the kritik stok seviyesi label
